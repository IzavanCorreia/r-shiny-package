...
-   [Conceitos Introdutórios: Pacotes em R e Aplicações
    Web](#Conceitos-Introdutórios-Pacotes-em-R-e-o-Shiny)
...
-   [O que é o Shiny?](#O-que-é-o-Shiny)
    -   [Entradas e saídas do Shiny](#Entradas-e-saídas-do-Shiny)
        ...
        -   [renders](#renders)
-   [Exemplo prático: Construção da primeira aplicação web com
    Shiny](#Exemplo-prático-Construção-da-primeira-aplicação-Web-com-Shiny)
...
base em **tecnologias** e **protocolos de comunicação** próprios da
**Web**, tais como HTTP, HTTPS, HTML, CSS e Javascript.

De forma mais simples: aplicações web levam informações de um lugar a
outro, contando com a ajuda de várias tecnologias.

Estas aplicações podem utilizar frameworks e bibliotecas derivados das
tecnologias já citadas, a exemplo da biblioteca React, destina à
construção de interfaces para **sites** renderizados em browsers de PC,
React Native, destinada ao desenvolvimento de **aplicativos móveis** e
Node.Js, destinada à criação de sistemas que rodam em servidores.

Estas aplicações precisam estar necessariamente conectadas à rede para
funcionarem.

O desenvolvimento deste tipo de sistema é dividido, de forma geral, em
desenvolvimento:

-   **front-end**, que se refere à porcão ***client-side*** e as
    **requisições** feitas por ele, e
-   **back-end**, que se refere ao ***server-side*** e as **respostas**
    que o cliente receberá a cada requisição.
...
Ou seja: cada vez que você clica na busca do Google, são as aplicações
web que “pegam” sua pesquisa, procuram internamente e te devolvem uma
resposta.
...
O Shiny trata-se de um sistema para desenvolvimento de **aplicações
web** usando o R, um pacote do R (shiny) e um servidor web (shiny
server).

> Observação: Não confundir o shiny com uma página web! Ele é uma
> ferramenta que torna a linguagem R mais robusta e versátil, ajudando a
...
com o app. Eles recebem um valor escolhido pelo usuário e este valor é
enviado para o server side.
